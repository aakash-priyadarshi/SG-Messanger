...
namespace WoWonder.SQLite
{
    public class SqLiteDatabase : IDisposable
    {
            ...
            try
            {
                using (Connection = OpenConnection())
                {
                    ...
                    else
                    {
                        ...
                        if (cec == null)
                        {
                            ChatWindowActivity.GetInstance()?.StartedMessageList.Add(new AdapterModelsClassUser()
                            {
                                Id = Long.ParseLong(item.Id),
                                MesData = item,
        ...
        public ObservableCollection<AdapterModelsClassUser> GetStartedMessageList(string fromId, string toId )
        {
            try
            {
                using (Connection = OpenConnection())
                {
                    ...
                    var list = new ObservableCollection<AdapterModelsClassUser>();
                    foreach (var item in query)
                    {
                        ...
                        list.Add(new AdapterModelsClassUser
                        {
                            TypeView = type,
                            Id = Long.ParseLong(item.Id),
                            MesData = WoWonderTools.MessageFilter(toId, maTb, type, true)
            ...
            {
                Console.WriteLine(e);
                return new ObservableCollection<AdapterModelsClassUser>();
            }
        }
