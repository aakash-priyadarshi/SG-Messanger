...
using WoWonder.Activities.PageChat.Adapter;
...
namespace WoWonder.Activities.PageChat
{
    ...
    { 
        ...
        private AdapterModelsClassPage SelectedItemPositions;
        ...
        public PageMessagesAdapter MAdapter;
        ...
        private void SetRecyclerViewAdapters()
        {
            try
            {
                MAdapter = new PageMessagesAdapter(this, PageId);
        ...
        private void MAdapterOnItemClick(object sender, Holders.MesClickEventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    else if (e.Type == Holders.TypeClick.Video)
                    {
                        var fileName = item.MesData.Media.Split('/').Last();
                    ...
                    else if (e.Type == Holders.TypeClick.Image)
                    {
                        var fileName = item.MesData.Media.Split('/').Last();
                        var mediaFile = WoWonderTools.GetFile(PageId, Methods.Path.FolderDcimVideo, fileName, item.MesData.Media);
        ...
        private void SuggestionAdapterOnOnItemClick(object sender, AdapterClickEvents e)
        {
            try
            {
                ...
                MAdapter.DifferList.Add(new AdapterModelsClassPage()
                {
                            ...
                            if (respond is PageMessagesObject result)
                            {
                                ...
                                if (respondList > 0)
                                {
                                    ...
                                    foreach (var item in result.Data)
                                    {
                                        ...
                                        var message = WoWonderTools.MessageFilter(PageId, item, type);
                        ...
                        if (respond is PageMessagesObject result)
                        {
                            ...
                            if (respondList > 0)
                            {
                                foreach (var item in from item in result.Data let check = MAdapter.DifferList.FirstOrDefault(a => a.MesData.Id == item.Id) where check == null select item)
                                {
                                    ...
                                    var mes = new AdapterModelsClassPage()
                                    {
                                        TypeView = type,
                                        Id = Long.ParseLong(item.Id),
                                        MesData = WoWonderTools.MessageFilter(PageId, item, type)
        ...
        {
            private PageChatWindowActivity ChatWindow;
