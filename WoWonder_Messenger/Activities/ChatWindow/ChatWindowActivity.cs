...
namespace WoWonder.Activities.ChatWindow
{
    ...
    public class ChatWindowActivity : AppCompatActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, View.IOnLayoutChangeListener
    {
        ...
        public ObservableCollection<AdapterModelsClassUser> StartedMessageList = new ObservableCollection<AdapterModelsClassUser>();
            ...
            try
            {
                if (TaskWork == "Working")
                {
                    ...
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        (int apiStatus, var respond) = await ApiRequest.FetchUserChatMessages(Userid, "0", "0","35");
                        if (apiStatus == 200)
                        {
                            if (respond is UserChatMessagesObject result)
                            {
                                ...
                                if (respondList > 0)
                                {
                                    foreach (var item in result.Messages)
                                    {
                                        ...
                                        var message = WoWonderTools.MessageFilter(Userid, item, type, true);
